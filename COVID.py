...
c = 1; #select country
m = 0; #Model ID
lambda1 = 1e-2	#Regularization on Infected cases
lambda2 = 1e-1 #Regularization on Recovered cases
...
def optimize(D):
	...
	if m ==0:
		...
		Do = D[-2:].astype(float)
		Cval.append(Do)
		res = minimize(SIR, Do, method='SLSQP',jac=FD_SIR,bounds=bnds1,options={'disp': True})
		#res = minimize(SIR, Do, method='Nelder-Mead',options={'disp': True})
		Cval.append(res.x)
	elif m ==1:
...
def main():
	...
	#	plt.text(Cval[0,0]+0.1*Cval[0,0],Cval[0,1]-0.2*Cval[0,1],str(Cval[0,:]))
	#	plt.text(Cval[-1,0]+0.1*Cval[-1,0],Cval[-1,1]-0.1*Cval[-1,1],str(Cval[-1,:]))
	#	plt.colorbar(cp)
	#	plt.title('Convergence contour plot (SIR Model, USA)')
	#	plt.xlabel('Beta')
	#	plt.ylabel('Gamma')
